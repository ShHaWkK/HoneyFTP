...
– Certificat TLS auto-signé (server.key/server.crt) dans un dossier inscriptible  
– FTPS implicite (SSL4ServerEndpoint)  
– Auth anonymous + attacker/secret  
– Canary-files + honeytoken/session  
– FS dynamique (dirs/fichiers aléatoires)  
– DELE → quarantaine + log MD5/SHA256  
– RNFR/RNTO → trace dans .rename.log  
– MKD/RMD → alertes sur répertoires “interdits”  
...
if os.access(script_dir, os.W_OK):
    ...
    print(f"[WARN] {script_dir} non-écrivable → {BASE}")
...
for d in (ROOT_DIR, QUAR_DIR, SESSIONS_DIR):
...
try:
    ...
    print(f"[WARN] impossible d’écrire le log central : {e}")
...
if not (os.path.exists(KEY_FILE) and os.path.exists(CRT_FILE)):
    logging.info("Génération d’un certificat auto-signé…")
...
class HoneyFTP(ftp.FTP):
    ...
    def ftp_RNFR(self, fn):
        peer=self.transport.getPeer().host; old=fn.lstrip("/")
        self._old=old
    ...
    def ftp_RNTO(self,new):
        peer=self.transport.getPeer().host; old=getattr(self,"_old",None)
        new=new.lstrip("/")
        if not old: return (ftp.SYNTAX_ERR,"RNFR first")
    ...
    def ftp_DELE(self,path):
        peer=self.transport.getPeer().host; rel=path.lstrip("/")
        tag=f"{self.session}_{uuid.uuid4().hex}"; dst=os.path.join(QUAR_DIR,tag)
        try:
            os.makedirs(os.path.dirname(dst),exist_ok=True)
    ...
    def ftp_RETR(self,path):
        rel=path.lstrip("/"); peer=self.transport.getPeer().host
        if rel in CANARY: alert(f"CANARY RETR {rel} by {peer}")
    ...
    def lineReceived(self,line):
        peer=self.transport.getPeer().host; cmd=line.decode("latin-1").strip()
